...
export function TabSegment<T extends string>({ tabs, value, onChange, className }: Props<T>) {
  ...
  return (
    <div
      role="tablist"
      className={cn(
        'relative inline-grid p-1 rounded-full glass shadow-soft',
        className,
      )}
      style={{ gridTemplateColumns: `repeat(${tabs.length}, minmax(0, 1fr))` }}
    >
      {/* 滑動指示器 — 紫色 fill 跟著 activeIdx 滑、寬度等於一格 */}
      <span
        aria-hidden="true"
        className="absolute top-1 bottom-1 left-1 rounded-full bg-primary shadow-soft transition-transform duration-300 ease-out"
        style={{
      ...
      {tabs.map((tab, idx) => {
        ...
        return (
          <button
            ...
            className={cn(
              'relative z-10 inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-full text-base transition-colors whitespace-nowrap',
              'min-h-[44px]',
              active
                ? 'text-on-primary'
