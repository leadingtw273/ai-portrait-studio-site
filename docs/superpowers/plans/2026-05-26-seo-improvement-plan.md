...
### Task 3：新增 `public/sitemap.xml` + `public/robots.txt`
...
- [ ] **Step 8.4：preview 驗證 redirect 行為**

```bash
pnpm preview --port 4173 &
sleep 3
# curl -L 跟隨 redirect
curl -s -L http://localhost:4173/ai-portrait-studio-site/ | grep -E "Mini Launch|hero-bg" | head -3
kill %1
```

Expected: 經 redirect 後 stdout 含三語版本內容（如 Mini Launch 文字或 hero-bg image）。
...
### Task 13：`AddOnsCarousel` 改 CSS-only responsive（移除 `window.innerWidth` JS state）
...
### Task 14：`useCurrency` SSR-safe + initial fallback 與 prerender 一致
...
若某 task 失敗：
1. **Phase 1-3 內 task 失敗**：revert 該 task 的 commit、production 不受影響（這幾個 phase 都 backward compatible）
2. **Phase 4 task 失敗（特別是 Task 16 prerender / Task 12 hydrateRoot）**：revert 該 task + Task 後續 task；production 仍可用 Phase 1-3 的成果
3. **Phase 5 CI/CD 失敗**：deploy.yml 改回前一版（git revert）即可、Pages 仍 serving 上一次成功 build
4. **災難級**：`git revert` 全部 SEO 相關 commit、production 回到 SEO 改造前狀態（site 仍 functional、只是 SEO 不佳）
